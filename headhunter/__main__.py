...
def get_areas():
    return requests.get(f'{.hhAPI_URL}/areas').json()


def get_cities():
    return requests.get(f'{hh.API_URL}/areas/{area_id}').json()['areas']
...
def get_vacancy(vacancy_id):
    ...
    resp = requests.get(f'{API_URL}/vacancies/{vacancy_id}')
    if resp.status_code != 200:
        raise Exception(f"Load vacancy error, id = {vacancy_id}!")
    info = resp.json()
    ...
    try:
        ...
        global excluded_vacancies
        excluded_vacancies += [vacancy_id]
